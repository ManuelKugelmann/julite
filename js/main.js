...
function pg_draw(id, angle, lux, luxD, minD, maxD, kelvin, value, canvas) {
  ...
  var infoObj = canvas.text( luxAtD(lux,luxD, value) + " lux at " + (value*0.01).toFixed(2)  + " m" );
  infoObj.fill({ color: '#fff' })
  infoObj.move(svgElement.height() * 0.1, svgElement.height() * 0.05);
  ...
  if(typeof value !='undefined' && value !== null && value != 0 && heightRefHeight != 0 ) {
    var scaleFactor = beamHeight / value;
    heightRef.size(heightRef.width() * 90/heightRefHeight, heightRefHeight * 90/heightRefHeight);
